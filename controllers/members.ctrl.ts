...
async function findByScreenName(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (screenName === undefined || screenName === null) {
    throw new BadReqError('screen name이 누락되었습니다.');
  }
  const extreatScreenName = Array.isArray(screenName) ? screenName[0] : screenName;
  const findResult = await MemberModel.findByScreenName(extreatScreenName);
  if (findResult === null) {
